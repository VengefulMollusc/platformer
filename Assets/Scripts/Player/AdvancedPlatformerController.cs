...
namespace CMF
{
	...
	public class AdvancedPlatformerController : Controller {
		...
		protected CharacterGrappleController grappleController;
		...
		//Time to allow late jump when moving off a ledge
		[Tooltip("Time to allow late jumping when moving off a ledge.")]
		[SerializeField]
		float lateJumpTime = 0.1f;
		float currentLateJumpTime = 0f;

		//Time to allow pre-loading a jump input before landing
		[Tooltip("Time to allow pre-loading a jump input before landing.")]
		[SerializeField]
		float preJumpTime = 0.2f;
		float currentPreJumpTime = 0f;
		...
		void Awake () {
			...
			grappleController = GetComponent<CharacterGrappleController>();
		...
		void HandleJumpKeyInput()
		{
			...
			if(jumpKeyIsPressed == false && _newJumpKeyPressedState == true)
			{
				...
				currentPreJumpTime = preJumpTime;
			}
		...
		void FixedUpdate()
		{
			...
			if (currentControllerState == ControllerState.Overridden)
			{
		...
		ControllerState DetermineControllerState()
		{
			...
			else if (currentControllerState == ControllerState.Overridden)
			{
		...
		{
			return grappleController && grappleController.IsGrappling();
		}

		//Check if player has initiated a jump;
		void HandleJumping()
		{
			if(currentControllerState == ControllerState.Grounded || currentLateJumpTime > 0f)
			{
				if(jumpKeyWasPressed || (jumpKeyIsPressed == true && currentPreJumpTime > 0f))
				{
					//Call events;
					OnGroundContactLost();
					OnJumpStart();
					...
					// reset jump timers
					currentLateJumpTime = 0f;
					currentPreJumpTime = 0f;

					currentControllerState = ControllerState.Jumping;
				}
			}

			// if late jump timer is active, decrement time available
			if (currentLateJumpTime > 0f)
				currentLateJumpTime -= Time.fixedDeltaTime;

			// if pre jump timer is active, decrement time available
			if (currentPreJumpTime > 0f)
				currentPreJumpTime -= Time.fixedDeltaTime;
		}
		...
		{
			// start late jump timer if not jumping
			if (currentControllerState != ControllerState.Jumping)
			{
				currentLateJumpTime = lateJumpTime;
			}
